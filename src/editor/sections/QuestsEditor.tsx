...
export function QuestsEditor() {
  ...
  const [selectedQuestId, setSelectedQuestId] = useState(project.quests[0]?.id ?? "");
  const selectedQuest = project.quests.find((quest) => quest.id === selectedQuestId);
  ...
  function deleteObjective(id: string) {
    ...
    updateQuest({ objectives: selectedQuest.objectives.filter((objective) => objective.id !== id) });
  }
  ...
  function addReward() {
    ...
    updateQuest({ rewards: [...(selectedQuest.rewards ?? []), makeReward("item")] });
  }
  ...
  function deleteReward(index: number) {
    updateQuest({ rewards: (selectedQuest?.rewards ?? []).filter((_, rewardIndex) => rewardIndex !== index) });
  }
  ...
  function renderCondition(objective: Objective) {
    ...
    return (
      <div className="quest-condition-row">
        <select
          ...
          onChange={(event) =>
            updateObjective(objective.id, {
              condition: makeCondition(event.target.value as ObjectiveCondition["type"]),
            })
          }
        ...
        {condition.type === "flag" ? (
          <>
            <select onChange={(event) => updateObjective(objective.id, { condition: { ...condition, flag: event.target.value } })} value={condition.flag}>
              {flagNames.map((flag) => <option key={flag} value={flag}>{flag}</option>)}
            </select>
            <select onChange={(event) => updateObjective(objective.id, { condition: { ...condition, value: event.target.value === "true" } })} value={String(condition.value)}>
        ...
        {condition.type === "has_item" ? (
          <>
            <select onChange={(event) => updateObjective(objective.id, { condition: { ...condition, itemId: event.target.value } })} value={condition.itemId}>
              {project.items.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
            </select>
            <input min={1} onChange={(event) => updateObjective(objective.id, { condition: { ...condition, quantity: Math.max(1, Number(event.target.value)) } })} type="number" value={condition.quantity ?? 1} />
          </>
        ) : null}
          ...
          <>
            <select
              onChange={(event) => {
                ...
                updateObjective(objective.id, { condition: { ...condition, variable, value: project.gameState.variables[variable] ?? 0 } });
              }}
              value={condition.variable}
            >
              {variableNames.map((variable) => <option key={variable} value={variable}>{variable}</option>)}
            </select>
            <select onChange={(event) => updateObjective(objective.id, { condition: { ...condition, operator: event.target.value as VariableComparisonOperator } })} value={condition.operator}>
              {comparisonOperators.map((operator) => <option key={operator} value={operator}>{operator}</option>)}
            </select>
            <input onChange={(event) => updateObjective(objective.id, { condition: { ...condition, value: readValue(event.target.value, condition.value) } })} type={typeof condition.value === "number" ? "number" : "text"} value={condition.value} />
          </>
        ) : null}
        {condition.type === "enter_area" ? (
          <select onChange={(event) => updateObjective(objective.id, { condition: { ...condition, areaId: event.target.value } })} value={condition.areaId}>
  ...
  function renderReward(reward: QuestReward, index: number) {
    return (
      <div className="quest-reward-row" key={`${selectedQuest?.id}_${index}`}>
        <select onChange={(event) => updateReward(index, makeReward(event.target.value as QuestReward["type"]))} value={reward.type}>
        ...
        {reward.type === "item" ? (
          <>
            <select onChange={(event) => updateReward(index, { ...reward, itemId: event.target.value })} value={reward.itemId}>
              {project.items.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
            </select>
            <input min={1} onChange={(event) => updateReward(index, { ...reward, quantity: Math.max(1, Number(event.target.value)) })} type="number" value={reward.quantity} />
          </>
        ) : null}
        {reward.type === "flag" ? (
          <>
            <select onChange={(event) => updateReward(index, { ...reward, flag: event.target.value })} value={reward.flag}>
              {flagNames.map((flag) => <option key={flag} value={flag}>{flag}</option>)}
            </select>
            <select onChange={(event) => updateReward(index, { ...reward, value: event.target.value === "true" })} value={String(reward.value)}>
              <option value="true">true</option>
              <option value="false">false</option>
            </select>
          </>
        ...
        {reward.type === "variable" ? (
          <>
            <select onChange={(event) => updateReward(index, { ...reward, variable: event.target.value })} value={reward.variable}>
              {variableNames.map((variable) => <option key={variable} value={variable}>{variable}</option>)}
            </select>
            <input onChange={(event) => updateReward(index, { ...reward, amount: Number(event.target.value) })} type="number" value={reward.amount} />
          </>
        ) : null}
        <button className="danger-button compact" onClick={() => deleteReward(index)} type="button">Delete</button>
      </div>
    );
  }
      ...
      <aside className="tool-panel">
        ...
        <div className="list-stack quest-list">
          {project.quests.map((quest) => (
            <button className={`list-item ${quest.id === selectedQuestId ? "selected" : ""}`} key={quest.id} onClick={() => setSelectedQuestId(quest.id)} type="button">
              <span>
        ...
        <label>
          Tracked quest
          <select onChange={(event) => updateProject((draft) => { draft.trackedQuestId = event.target.value || undefined; })} value={project.trackedQuestId ?? ""}>
        ...
        {selectedQuest ? (
          <>
            ...
            <div className="form-grid">
              <label>
                Name
                <input onChange={(event) => updateQuest({ name: event.target.value })} value={selectedQuest.name} />
              </label>
              <label>
                Default status
                <select onChange={(event) => updateQuest({ status: event.target.value as Quest["status"] })} value={selectedQuest.status}>
            ...
            <label>
              Description
              <textarea onChange={(event) => updateQuest({ description: event.target.value })} rows={3} value={selectedQuest.description ?? ""} />
            </label>
            <section className="quest-section">
              ...
              {selectedQuest.objectives.map((objective) => (
                <div className="quest-objective" key={objective.id}>
                  <input onChange={(event) => updateObjective(objective.id, { description: event.target.value })} value={objective.description} />
          ...
          <p className="empty-state">Add a quest to define player-facing objectives.</p>
        )}
      </div>
    </section>
  );
