...
const categories: ItemDefinition["category"][] = [
	"key",
	"currency",
	"consumable",
	"quest",
...
function conditionReferencesItem(
	...
	if (expression.type === "group") {
		return expression.conditions.some((condition) =>
			conditionReferencesItem(condition, itemId),
		);
	}
		...
		const questReference = project.quests.some(
			(quest) =>
				quest.objectives.some(
					(objective) =>
						objective.condition.type === "has_item" &&
						objective.condition.itemId === selectedItem.id,
				) ||
				quest.rewards?.some(
					(reward) =>
						reward.type === "item" && reward.itemId === selectedItem.id,
				),
		);
		...
		<section className="editor-panel items-editor">
			<aside className="tool-panel">
				...
				<p className="helper-text">
					Definitions used by pickups, quests, and rules.
				</p>
				<button
				...
				<div className="list-stack item-list">
					...
					{project.items.length === 0 ? (
						<p className="empty-state compact">No items defined.</p>
					) : null}
				</div>
			</aside>
			...
			<div className="content-panel">
				{selectedItem ? (
									...
									onChange={(event) =>
										updateItem({
											category: event.target
												.value as ItemDefinition["category"],
										})
									}
						...
						{selectedItem.stackable ? (
							<label>
								...
								<input
									...
									onChange={(event) =>
										updateItem({
											maxStack: Math.max(1, Number(event.target.value)),
										})
									}
					...
					<p className="empty-state">
						Add an item definition to use inventory rules and pickups.
					</p>
				)}
			</div>
		</section>
