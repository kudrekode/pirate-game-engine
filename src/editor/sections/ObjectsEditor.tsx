...
export function isObjectDefinitionPlaced(
	...
	return areas.some((area) =>
		area.objects.some(
			(object) => object.objectDefinitionId === objectDefinitionId,
		),
	);
}
...
export function ObjectsEditor() {
	...
	function deleteObject() {
		...
		if (isObjectDefinitionPlaced(project.areas, selectedObject.id)) {
			setMessage(
				`${selectedObject.name} is placed in a map. Delete those instances first.`,
			);
			return;
		}
		...
		<section className="editor-panel items-editor">
			<aside className="tool-panel">
				...
				<p className="helper-text">
					Reusable props placed as map object instances.
				</p>
				<button
				...
				<div className="list-stack item-list">
					{project.objects.map((object) => (
						...
						>
							<span className="item-icon">
								{object.name.slice(0, 1).toUpperCase()}
							</span>
							<span>
					...
					{project.objects.length === 0 ? (
						<p className="empty-state compact">No object definitions.</p>
					) : null}
				</div>
			</aside>
			...
			<div className="content-panel">
				{selectedObject ? (
					<>
						...
						<p className="helper-text">
							Objects are generic map entities between structures and NPCs.
						</p>
						{message ? (
									...
									onChange={(event) =>
										updateObject({
											category: event.target
												.value as ObjectDefinition["category"],
										})
									}
						...
						<div className="form-grid compact">
							<label>
								...
								<input
									...
									type="number"
									value={selectedObject.widthTiles}
								/>
							</label>
							<label>
								Height tiles
								<input
									...
									type="number"
									value={selectedObject.heightTiles}
								/>
							</label>
						</div>
						<label className="checkbox-row standalone">
							<input
								checked={selectedObject.blocksMovement}
								onChange={(event) =>
									updateObject({ blocksMovement: event.target.checked })
								}
								type="checkbox"
							/>
						...
						<ObjectBehaviourEditor
							behaviour={
								selectedObject.defaultBehaviour ??
								makeDefaultObjectBehaviour("none")
							}
							onChange={(behaviour) =>
								updateObject({ defaultBehaviour: behaviour })
							}
							project={project}
						/>
					...
					<p className="empty-state">
						Add an object definition to place generic props in maps.
					</p>
				)}
			</div>
		</section>
