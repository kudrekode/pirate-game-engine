...
function removeFlagReferences(project: GameProject, flag: string) {
	project.rules = project.rules.map((rule) => ({
		...
		actions:
			removeActions(
				rule.actions,
				(action) => action.type === "set_flag" && action.flag === flag,
			) ?? [],
	...
	project.areas.forEach((area) => {
		area.eventBlocks.forEach((eventBlock) => {
			if (
				eventBlock.interaction?.type === "set_flag" &&
				eventBlock.interaction.flag === flag
			) {
		...
		area.structures.forEach((structure) => {
			if (
				structure.interaction?.type === "set_flag" &&
				structure.interaction.flag === flag
			) {
		...
		area.objects.forEach((object) => {
			if (
				object.interaction?.type === "set_flag" &&
				object.interaction.flag === flag
			) {
...
export function GameStateEditor() {
	...
	function addFlag() {
		...
		const error = validateIdentifier(
			name,
			project.gameState.flags,
			project.gameState.variables,
		);
		if (error) {
			...
			<div className="content-panel">
				...
				<section className="state-section">
					...
					<div className="state-list">
						...
						{Object.keys(project.gameState.flags).length === 0 ? (
							<p className="empty-state compact">No flags defined.</p>
						) : null}
					</div>
				</section>
				...
				<section className="state-section">
					...
					<div className="state-list">
						{Object.entries(project.gameState.variables).map(
							([name, value]) => (
						...
						{Object.keys(project.gameState.variables).length === 0 ? (
							<p className="empty-state compact">No variables defined.</p>
						) : null}
					</div>
				</section>
			</div>
			...
			<aside className="inspector-panel">
				...
				<div className="form-stack">
					...
					<label>
						...
						<select
							onChange={(event) =>
								setNewVariableType(event.target.value as VariableType)
							}
