...
function updateExpression(
	...
	return {
		...
		conditions: expression.conditions.map((condition) =>
			updateExpression(condition, id, updater),
		),
	};
}
...
export function ProgressionEditor() {
	...
	useEffect(() => {
		if (
			!selectedRule ||
			!project.rules.some((rule) => rule.id === selectedRuleId)
		) {
	...
	function updateRule(rule: GameRule) {
		updateProject((draft) => {
			draft.rules = draft.rules.map((candidate) =>
	...
	function addRootCondition(rule: GameRule, condition: ConditionExpression) {
		if (!rule.conditionTree) {
			updateRule({
				...rule,
				conditionTree: { ...makeConditionGroup(), conditions: [condition] },
			});
			return;
		}
		...
		updateRule({
			...
			conditionTree: {
				...makeConditionGroup(),
				conditions: [rule.conditionTree, condition],
			},
		});
	}
	...
	function addGroupCondition(
		...
		updateRule({
			...rule,
			conditionTree: updateExpression(
				rule.conditionTree,
				groupId,
				(expression) =>
					expression.type === "group"
						? {
								...expression,
								conditions: [...expression.conditions, condition],
							}
						: expression,
			),
		});
			...
			<div className="logic-sentence">
				...
				<select
					onChange={(event) =>
						updateRule({
							...rule,
							trigger: makeTrigger(event.target.value as RuleTrigger["type"]),
						})
					}
				...
				{rule.trigger.type === "on_interact" ? (
					<select
						onChange={(event) =>
							updateRule({
								...rule,
								trigger: { type: "on_interact", targetId: event.target.value },
							})
						}
				...
				{rule.trigger.type === "on_touch" ? (
					<select
						onChange={(event) =>
							updateRule({
								...rule,
								trigger: { type: "on_touch", targetId: event.target.value },
							})
						}
						value={rule.trigger.targetId}
					>
				...
				{rule.trigger.type === "on_area_enter" ? (
					<select
						onChange={(event) =>
							updateRule({
								...rule,
								trigger: { type: "on_area_enter", areaId: event.target.value },
							})
						}
	...
	function renderCondition(
		...
		return (
			<div
				className="logic-row condition-row"
				key={condition.id}
			...
			>
				<select
					onChange={(event) =>
						replaceCondition(
							...
							condition.id,
							makeCondition(
								event.target.value as SingleCondition["type"],
								condition.id,
							),
						)
					}
				...
				<button
					...
					onClick={() =>
						updateRule({
							...rule,
							conditionTree: removeExpression(rule.conditionTree, condition.id),
						})
					}
					type="button"
				>
	...
	function renderConditionExpression(
		...
		return (
			<div
				className={`condition-group ${depth > 0 ? "nested" : ""}`}
				key={expression.id}
			>
				<div className="condition-group-heading">
					...
					<button
						onClick={() =>
							addGroupCondition(rule, expression.id, makeCondition("flag_is"))
						}
						type="button"
					>
						Add condition
					...
					<button
						onClick={() =>
							addGroupCondition(rule, expression.id, makeConditionGroup())
						}
						type="button"
					>
						Add nested group
					...
					{depth > 0 ? (
						<button
							...
							onClick={() =>
								updateRule({
									...rule,
									conditionTree: removeExpression(
										rule.conditionTree,
										expression.id,
									),
								})
				...
				{expression.conditions.length === 0 ? (
					<div className="logic-helper">Always run</div>
				) : null}
	...
	function renderAction(
		...
		const actions =
			branch === "actions" ? rule.actions : (rule.elseActions ?? []);
		...
		return (
			<div className="logic-row" key={`${rule.id}_${branch}_${index}`}>
				<select
					onChange={(event) =>
						setAction(makeAction(event.target.value as GameAction["type"]))
					}
					value={action.type}
				>
				...
				{action.type === "teleport" ? (
					<>
						...
						>
							{(
								project.areas.find((area) => area.id === action.areaId)
									?.eventBlocks ?? []
							).map((eventBlock) => (
				...
				{action.type === "change_movement_mode" ? (
					<select
						onChange={(event) =>
							setAction({
								...action,
								mode: event.target.value as "walk" | "sail" | "ride",
							})
						}
						value={action.mode}
					>
				...
				{action.type === "set_npc_alignment" ? (
					<>
						<select
							onChange={(event) =>
								setAction({ ...action, npcId: event.target.value })
							}
							value={action.npcId}
						>
							{npcTargets.map((npc) => (
	...
	function renderActions(
		...
		return (
			<section className="logic-block">
				...
				<button
					onClick={() =>
						updateActions(rule, branch, [...actions, makeAction("set_flag")])
					}
					type="button"
				>
					Add action
	...
	function rulesForFolder(groupId?: string) {
		return project.rules.filter((rule) =>
			groupId
				...
				: !rule.groupId || !folderIds.has(rule.groupId),
		);
	}
			...
			<section className="logic-folder" key={group.id}>
				<div className="logic-folder-heading">
					<button
						...
						onClick={() =>
							updateFolder(group.id, { collapsed: !group.collapsed })
						}
					...
					<input
						...
						onChange={(event) =>
							updateFolder(group.id, { name: event.target.value })
						}
						value={group.name}
					/>
					<button
				...
				{!group.collapsed ? (
					<div className="logic-folder-rules">
						...
						<button onClick={() => createRule(group.id)} type="button">
							Add rule
						</button>
					</div>
				) : null}
		...
		<section className="editor-panel progression-editor">
			<aside className="tool-panel logic-rule-list">
				...
				<p className="helper-text">
					WHEN / IF / THEN rules. Folders only organise the editor.
				</p>
				<button
				...
				{folderError ? (
					<div className="validation-message">{folderError}</div>
				) : null}
				...
				<section className="logic-folder">
					...
					<div className="logic-folder-rules">
						...
						{rulesForFolder().length === 0 ? (
							<p className="empty-state compact">No ungrouped rules.</p>
						) : null}
					</div>
				</section>
			</aside>
					...
					<>
						...
						<div className="form-grid compact">
							<label>
								...
								<input
									onChange={(event) =>
										updateRule({ ...selectedRule, name: event.target.value })
									}
						...
						<section className="logic-block">
							...
							{selectedRule.conditionTree ? (
								...
								<div className="logic-helper">
									Always run. Add a condition when this rule should be
									selective.
								</div>
							)}
							{!selectedRule.conditionTree ||
							...
							{selectedRule.conditionTree ? (
								<button
									...
									onClick={() =>
										updateRule({ ...selectedRule, conditionTree: undefined })
									}
									type="button"
								>
									Clear conditions
						...
						<div className="inline-actions">
							<button onClick={() => duplicateRule(selectedRule)} type="button">
								Duplicate rule
							</button>
							<button
								className="danger-button"
						...
						<details className="legacy-progression">
							<summary>
								Legacy linear progression ({project.progression.length} steps)
							</summary>
							<p>
								Preserved for existing projects while friendly rules are
								introduced.
							</p>
						</details>
					</>
