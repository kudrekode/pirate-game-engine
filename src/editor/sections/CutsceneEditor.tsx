...
export function CutsceneEditor() {
  ...
  const [selectedCutsceneId, setSelectedCutsceneId] = useState(cutscenes[0]?.id ?? "");
  ...
  function handleDeleteCutscene() {
    ...
    setSelectedCutsceneId(cutscenes.find((cutscene) => cutscene.id !== selectedCutscene.id)?.id ?? "");
  }
  ...
  return (
    <section className="editor-panel cutscene-editor">
      <aside className="tool-panel">
        ...
        <button className="primary-button full-width" onClick={handleAddCutscene} type="button">
          Add cutscene
        </button>
      </aside>
          ...
          <div className="cutscene-edit-layout">
            <div className="form-stack">
              <label>
                ...
                <input
                  onChange={(event) =>
                    updateCutscene(selectedCutscene.id, { name: event.target.value })
                  }
              ...
              <label>
                ...
                <input
                  onChange={(event) =>
                    updateCutscene(selectedCutscene.id, { speakerName: event.target.value })
                  }
              ...
              <label>
                ...
                <textarea
                  onChange={(event) =>
                    updateCutscene(selectedCutscene.id, { text: event.target.value })
                  }
                  rows={7}
                  value={selectedCutscene.text}
                />
              </label>
              <button className="danger-button" onClick={handleDeleteCutscene} type="button">
                Delete cutscene
              </button>
            </div>
              ...
              <div
                ...
                style={{
                  background: backgroundPresets.find(
                    (background) => background.id === selectedCutscene.backgroundImageId,
                  )?.color,
                }}
              >
                  ...
                  <span
                    ...
                    style={{
                      background: portraitPresets.find(
                        (portrait) => portrait.id === selectedCutscene.portraitImageId,
                      )?.color,
                    }}
                  />
                ) : null}
