...
function compareValues(left: number | string, right: number | string, operator: string): boolean {
  if (operator === "==") {
    return left === right;
  }
...
export function markAreaEntered(state: RuntimeQuestState, areaId: string): void {
...
export function evaluateObjectiveCondition(
  ...
  return value === undefined ? false : compareValues(value, condition.value, condition.operator);
}
...
export function evaluateObjective(
  ...
  return evaluateObjectiveCondition(objective.condition, runtimeState, enteredAreaIds);
}
...
export function evaluateQuest(
  ...
  return quest.objectives.length > 0 &&
    quest.objectives.every((objective) => evaluateObjective(objective, runtimeState, enteredAreaIds));
}
...
export function grantQuestReward(
  ...
  if (reward.type === "item") {
    giveItem(runtimeState.inventory, itemDefinitions, reward.itemId, reward.quantity);
    return;
  }
...
export function completeQuest(
  ...
  quest.rewards?.forEach((reward) => grantQuestReward(reward, runtimeState, itemDefinitions));
  state.rewardedQuestIds.add(quest.id);
  return true;
}

export function activateQuest(state: RuntimeQuestState, questId: string): boolean {
  const quest = state.quests.find((candidate) => candidate.id === questId);
...
export function getQuestViews(
  ...
  return state.quests.map((quest) => ({
    ...
    objectives: quest.objectives.map((objective) => ({
      ...
      complete: state.completedObjectiveIds.has(objectiveKey(quest.id, objective.id)),
    })),
  }));
}
