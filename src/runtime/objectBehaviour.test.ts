...
const items: ItemDefinition[] = [
  { id: "gold_coin", name: "Gold Coin", category: "currency", stackable: true, maxStack: 999 },
...
describe("object behaviours", () => {
  it("gives container contents once", () => {
    ...
    expect(runObjectBehaviour(behaviour, context)).toMatchObject({ type: "container", opened: true });
    expect(context.state.inventory.items.gold_coin).toBe(5);
    expect(context.state.flags.chest_opened).toBe(true);

    expect(runObjectBehaviour(behaviour, context)).toMatchObject({ type: "container", opened: false });
    expect(context.state.inventory.items.gold_coin).toBe(5);
  });
  ...
  it("blocks a door when required item is missing", () => {
    ...
    expect(result).toMatchObject({ type: "door", allowed: false, lockedCutsceneId: "locked" });
  });
  ...
  it("allows a door when required item is present", () => {
    ...
    expect(result).toMatchObject({ type: "door", allowed: true, targetAreaId: "tavern", targetEventBlockId: "entry" });
  });

  it("returns sign text", () => {
    expect(runObjectBehaviour({ type: "sign", text: "Read me." }, makeContext())).toEqual({
      type: "sign",
      handled: true,
      text: "Read me.",
    });
