...
export type VehicleMovementConfig = Extract<ObjectBehaviour, { type: "vehicle" }>;
...
function overlayIdAt(area: GameArea, x: number, y: number): string | undefined {
  return area.overlayTiles.find((tile) => tile.x === x && tile.y === y)?.overlayId;
}
...
export function canDismountAt(area: GameArea, x: number, y: number, vehicle: VehicleMovementConfig): boolean {
  if (x < 0 || y < 0 || x >= area.width || y >= area.height) {
    return false;
  }
...
function isBlockedForDismount(area: GameArea, x: number, y: number): boolean {
  ...
  if (area.objects.some((object) => object.blocksMovement === true && objectCoversTile(object, area, x, y))) {
    return true;
  }

  return area.npcs.some((npc) => npc.blocksMovement && npc.x === x && npc.y === y);
}
