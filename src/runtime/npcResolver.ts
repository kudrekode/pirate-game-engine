...
function mergeMovement(
  ...
  const legacyMovement = definition
    ...
    : {
        ...
        movementSpeed: instance.movementSpeed ?? instance.attributes.movementSpeed,
...
export function resolveNPCInstance(
  ...
  const enemyBase = definition ? definition.defaultEnemyBehaviour : instance.enemyBehaviour;
  ...
  return {
    ...
    movementSpeed: Math.max(0.1, movement.movementSpeed ?? attributes.movementSpeed ?? 1),
    patrolPath: movement.patrolPath,
    wanderZone: movement.wanderZone,
    enemyBehaviour,
    interaction: instance.interactionOverride ?? instance.interaction ?? definition?.defaultInteraction,
  };
}
