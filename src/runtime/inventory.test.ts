...
import { collectPickup, createInventory, giveItem, hasItem, removeItem } from "./inventory";

const items: ItemDefinition[] = [
  { id: "gold_coin", name: "Gold Coin", category: "currency", stackable: true, maxStack: 10 },
...
describe("inventory", () => {
  ...
  it("collects a pickup only once when configured", () => {
    ...
    expect(collectPickup(pickup, inventory, items, collectedPickupIds)).toBe(true);
    expect(collectPickup(pickup, inventory, items, collectedPickupIds)).toBe(false);
