...
describe("NPC movement helpers", () => {
	it("keeps stationary NPCs in place", () => {
		expect(updateStationaryNPC(makeNpc())).toMatchObject({
			moved: false,
			x: 1,
			y: 1,
			facing: "down",
	...
	it("progresses through patrol points and updates facing", () => {
		...
		const second = updatePatrolNPC(
			{ ...npc, x: first.x, y: first.y },
			first.state,
		);
		expect(second).toMatchObject({ moved: true, x: 3, y: 1 });

		const stopped = updatePatrolNPC(
			{ ...npc, x: second.x, y: second.y },
			second.state,
		);
	...
	it("generates wander destinations inside a clamped zone", () => {
		...
		expect(generateWanderDestination(npc, makeArea(), () => 0.99)).toEqual({
			x: 3,
			y: 3,
		});
	});
	...
	it("walks toward wander destinations and skips blocked steps", () => {
		...
		expect(
			updateWanderNPC(npc, makeArea(), {
				patrolIndex: 0,
				wanderTarget: { x: 3, y: 1 },
			}),
		).toMatchObject({
			moved: true,
			x: 2,
		...
		expect(
			updateWanderNPC(
				npc,
				makeArea(),
				{ patrolIndex: 0, wanderTarget: { x: 3, y: 1 } },
				() => false,
			).moved,
		).toBe(false);
	});
	...
	it("returns toward origin when player leaves chase radius", () => {
		...
		expect(
			updateEnemyNPC(
				...
				{ x: 8, y: 1 },
				{ x: 1, y: 1 },
				{ patrolIndex: 0, enemyChasing: true },
			),
		).toMatchObject({
			moved: true,
			x: 2,
	...
	it("detects enemy contact and applies contact damage", () => {
		...
		expect(
			applyEnemyContactDamage(100, npc.enemyBehaviour?.contactDamage),
		).toBe(90);
	});
