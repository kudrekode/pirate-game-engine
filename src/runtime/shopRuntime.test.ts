...
const items: ItemDefinition[] = [
  ...
  { id: "rum_bottle", name: "Rum Bottle", category: "consumable", stackable: true, maxStack: 12 },
];
...
describe("shop runtime", () => {
  it("buys an item when the player has enough currency", () => {
    ...
    expect(buyShopEntry(shop, "key", inventory, items, stocks.general_store)).toMatchObject({
      success: true,
    });
  ...
  it("fails when the player does not have enough currency", () => {
    ...
    expect(buyShopEntry(shop, "key", inventory, items, stocks.general_store)).toEqual({
      success: false,
  ...
  it("decrements runtime stock and blocks stock zero purchases", () => {
    ...
    expect(buyShopEntry(shop, "key", inventory, items, stocks.general_store).success).toBe(true);
