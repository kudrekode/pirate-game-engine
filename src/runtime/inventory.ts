import type { InventoryState, ItemDefinition, PickupObject } from "../types/game";

function normalizedQuantity(quantity: number): number {
  return Math.max(0, Math.round(Number.isFinite(quantity) ? quantity : 0));
}

function findItem(items: ItemDefinition[], itemId: string): ItemDefinition | undefined {
  return items.find((item) => item.id === itemId);
}

export function createInventory(items: Record<string, number> = {}): InventoryState {
  return {
    items: Object.fromEntries(
      Object.entries(items).map(([itemId, quantity]) => [itemId, normalizedQuantity(quantity)]),
    ),
  };
}

export function hasItem(inventory: InventoryState, itemId: string, quantity = 1): boolean {
  return (inventory.items[itemId] ?? 0) >= Math.max(1, normalizedQuantity(quantity));
}
...
export function removeItem(
  ...
  const nextQuantity = Math.max(0, (inventory.items[itemId] ?? 0) - normalizedQuantity(quantity));
