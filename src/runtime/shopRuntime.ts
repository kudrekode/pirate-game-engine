import type { InventoryState, ItemDefinition, ShopDefinition } from "../types/game";
...
export function createRuntimeShopStocks(shops: ShopDefinition[]): RuntimeShopStocks {
  return Object.fromEntries(
    shops.map((shop) => [
      shop.id,
      Object.fromEntries(
        shop.entries.flatMap((entry) =>
          entry.stock === undefined ? [] : [[entry.id, Math.max(0, Math.round(entry.stock))]],
        ),
      ),
    ]),
  );
...
export function buyShopEntry(
  ...
  if (entry.stock !== undefined) {
    stockByEntryId[entry.id] = Math.max(0, (stockByEntryId[entry.id] ?? entry.stock) - 1);
  }

  const item = itemDefinitions.find((candidate) => candidate.id === entry.itemId);
